...
public class Boj_1600_말이되고픈원숭이3 {
		...
		while(!queue.isEmpty()) {
			...
			while(size-->0) {
				...
				for (int d = 0; d < 8; d++) {
					...
					if (visited[nexti][nextj] > depth && map[nexti][nextj] == 0 && current.k>0) {
						queue.offer(new Point(nexti, nextj, current.k - 1));
						visited[nexti][nextj] = depth;
					}
				}
				...
				for (int d = 0; d < 4; d++) {
					...
					if(visited[nexti][nextj] > depth && map[nexti][nextj]==0) {
